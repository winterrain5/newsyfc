...
struct calendartab: View {
    ...
    func submitentry(currentuserid: Int, token: String, calendaractivity: String, availabilityid: Int) -> Promise<Void>{
        Promise.init { resolver in
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                guard let data = data, error == nil else {
                    print("student-availability-detail-update went wrong")
                    resolver.reject(error!)
                    return
                }
    ...
    func deleteslot(currentuserid: Int, token: String, availabilityid: Int) -> Promise<Void>{
        Promise.init { resolver in
            let formatter = DateFormatter()
            formatter.dateFormat = "yyyy-MM-dd"
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                guard let data = data, error == nil else {
                    print("student-availability-delete went wrong")
                    resolver.reject(error!)
                    return
                }
        ...
        for dates in mainschedulelist {
            if selecteddate == formatter.date(from: dates.lesson_date ) {
                if calendaractivity == "editing"{
                    datechecker += 1
                    wave01state = dates.wave_one
                    wave02state = dates.wave_two
                    wave03state = dates.wave_three
                    wave04state = dates.wave_four
